...
class CompletePatchProcessor:
    ...
    def __init__(self, 
                 ...
                 fda_beta: float = 0.1,
                 use_fda: bool = True,
                 use_hist_norm: bool = True,
                 use_bg_mask: bool = True):
        
        self.patch_size = patch_size
        self.stride = stride
        self.use_fda = use_fda
        self.use_hist_norm = use_hist_norm
        ...
        if self.use_fda and fda_reference_dir:
            if not os.path.exists(fda_reference_dir):
                print(f"ERROR: FDA Reference path does not exist: {fda_reference_dir}")
                self.use_fda = False # Auto-disable to avoid confusion
            else:
                self.fda_transform = FDATransform(
                    reference_images_path=fda_reference_dir,
                    beta_limit=fda_beta,
                    always_apply=True
                )
    ...
    def process_single_image(self, 
        ...
        # Step 1: Apply Histogram Normalization
        if self.use_hist_norm:
            print(f"Applying histogram normalization to {base_name}")
            try:
                processed_img = self.hist_norm(image=img_rgb)['image']
...
def run_complete_patch_pipeline(img_dir: str, 
    ...
    print(f"Histogram normalization: {'Enabled' if use_hist_norm else 'Disabled'}")
    ...
    if use_fda:
        ...
        print(f"FDA reference directory: {real_reference_dir}")
    print("=" * 60)
